...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE) {
  console.warn('Supabase not configured for admin announcements')
}
...
async function handler(req: Request) {
  try {
    // Admin auth: require x-admin-key header
    ...
    if (req.method === 'POST') {
      ...
      const { title, body: b, active } = body as any
      const { data, error } = await supabase.from('announcements').insert([{ title, body: b, active: active ?? true }]).select()
      if (error) return new Response(JSON.stringify({ error: error.message }), { status: 500, headers: { 'Content-Type': 'application/json' } })
