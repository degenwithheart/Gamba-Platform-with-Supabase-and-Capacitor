...
import useReportingToast from './hooks/useReportingToast'
import { useUserStore } from './hooks/useUserStore'
import reportClientError from './services/monitoringReporter'

import Dashboard from './sections/Dashboard/Dashboard'
import Game from './sections/Game/Game'
import AdminPage from './sections/Admin'
import Header from './sections/Header'
import ErrorBoundary from './components/ErrorBoundary'
import RecentPlays from './sections/RecentPlays/RecentPlays'
import Toasts from './sections/Toasts'
import TrollBox from './components/TrollBox'
import { ProfileFlow } from './sections/ProfileFlow'
...
function ErrorHandler() {
  ...
  useTransactionError((err) => {
    if (err.message === 'NOT_CONNECTED') {
      ...
      // Report transaction errors to monitoring (reporting toast will also try to report)
      try { reportClientError({ type: 'transaction', message: err.message, details: err.error ?? null, timestamp: Date.now() }) } catch (e) {}
      toast({ title: '❌ Transaction error', description: err.error?.errorMessage ?? err.message, level: 'error' })
    }
  })
  ...
  return (
    <>
      ...
      <ErrorBoundary>
        ...
        <MainWrapper>
  {ENABLE_SUPABASE_AUTH && <AnnouncementBanner />}
        <Routes>
          {/* Normal landing page always shows Dashboard (with optional inline game) */}
          <Route path="/"          element={<Dashboard />} />
          {/* Profile page - only if Supabase is enabled */}
